...
const getLowStockResponse = async (req) => {
...
const getSummaryResponse = async (req) => {
  ...
  const [todayOrders, totalInventories, lowStockCount, todayMovements] =
    await Promise.all([
      Order.find(companyQuery(req, { createdAt: { $gte: today } })),
      Inventory.countDocuments(companyQuery(req)),
      Inventory.countDocuments({
        company: getCompanyId(req),
        $expr: { $lt: ["$currentStock", "$minimumStock"] },
      }),
      StockMovement.countDocuments(companyQuery(req, { createdAt: { $gte: today } })),
    ]);
...
const applyStockMovement = async (req, action) => {
  ...
  const [product, inventory] = await Promise.all([
    Product.findOne(companyQuery(req, { _id: productId })),
    Inventory.findOne(companyQuery(req, { product: productId })),
  ]);
...
const getChatbotContext = async (req) => {
  ...
  const [lowStockItems, todayOrdersCount, todayMovements] = await Promise.all([
    Inventory.find({
      ...
      .populate("product", "name")
      .limit(5),
    Order.countDocuments(companyQuery(req, { createdAt: { $gte: today } })),
    StockMovement.countDocuments(companyQuery(req, { createdAt: { $gte: today } })),
  ]);
  ...
  try {
    ...
    if (usedGemini) {
      ...
      if (
        ...
        result = await getLowStockResponse(req);
      } else if (
