...
export default function Returns() {
    ...
    const loadData = async () => {
      try {
        ...
        const [returnsResponse, ordersResponse, productsResponse] =
          await Promise.all([getReturns(), getSalesOrders(), getProductListing()]);
  ...
  const stats = useMemo(
    () => [
      ...
      {
        label: "Damaged / Loss",
        value: returns.filter((item) => item.condition !== "restockable").length,
        icon: ShieldAlert,
      },
      {
        label: "Units Reviewed",
        value: returns.reduce((sum, item) => sum + (Number(item.quantity) || 0), 0),
        icon: AlertTriangle,
      },
    ],
    [returns],
  ...
  const handleFormChange = (event) => {
    ...
    setForm((current) => {
      ...
      if (name === "order") {
        ...
        next.product = order?.orderItems?.[0]?.product?._id || products[0]?._id || "";
      }
                  ...
                  filteredReturns.map((item) => (
                    <tr key={item._id}>
                      ...
                      <td>
                        <span
                          className={`ops-badge ${
                            item.condition === "restockable" ? "delivered" : "suspended"
                          }`}
                        >
          ...
          <div className="ops-modal-backdrop" onClick={closeModal}>
            <div className="ops-modal" onClick={(event) => event.stopPropagation()}>
              <div className="ops-modal-head">
                <div>
                  <h2>Record Return</h2>
                  <p>Choose whether this return becomes sellable stock again.</p>
                </div>
              ...
              <form className="ops-form" onSubmit={handleCreateReturn}>
                <label>
                  ...
                  <select name="order" value={form.order} onChange={handleFormChange}>
                  ...
                  >
                    ...
                    {selectableProducts.map((product) => (
                      <option key={product.id} value={product.id}>
                        {product.name}
                        {product.maxQuantity ? ` (${product.maxQuantity} ordered)` : ""}
                      </option>
                    ))}
                  </select>
                </label>
